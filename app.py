...
if files:
    ...
    if len(dfs) >= 2:
        ...
        names = list(dfs.keys())
        a = st.selectbox("Archivo A", names, index=0)
        b = st.selectbox("Archivo B", names, index=1)
        num_cols_a = [c for c in dfs[a].columns if pd.api.types.is_numeric_dtype(dfs[a][c])]
        num_cols_b = [c for c in dfs[b].columns if pd.api.types.is_numeric_dtype(dfs[b][c])]
        common = sorted(list(set(num_cols_a) & set(num_cols_b)))
        if common:
            metric = st.selectbox("Métrica", common, index=0)
            # unir por índice mínimo, temporalmente
            min_len = min(len(dfs[a]), len(dfs[b]))
            x = dfs[a][metric].reset_index(drop=True).iloc[:min_len]
            y = dfs[b][metric].reset_index(drop=True).iloc[:min_len]
            r, p = pearson_r(x, y)
            st.write(f"**RMSE:** {rmse(x, y):.4f} | **MAE:** {mae(x, y):.4f} | **r (Pearson):** {r:.4f}")
            st.plotly_chart(fig_scatter_identity(x, y, f"{metric} ({a})", f"{metric} ({b})"), use_container_width=True)
        else:
            st.info("No hay columnas numéricas en común para comparar aún.")
else:
...
if data:
    ...
    for tab, fname in zip(tabs_files, data.keys()):
        with tab:
            ...
            # Selector de columnas a analizar
            sel_cols = st.multiselect(
                "Selecciona columnas (ojos) a visualizar",
                options=num_cols_all,
                default=num_cols_all[:min(6, len(num_cols_all))],
                key=f"sel_{fname}"
            )
            ...
            # -------------------------
            # 3) Barplot de promedios (por columna) con escala uniforme
            # -------------------------
            st.markdown("### Promedios por columna (después de filtrar ceros)")
            means = df0[sel_cols].mean(numeric_only=True)
            fig_bar, ax_bar = plt.subplots(figsize=(10, 4))
            bars = ax_bar.bar(range(len(means)), means.values, color="#FFA500")  # naranja
            
            # Marcar máximo y mínimo
            max_idx = means.idxmax()
            min_idx = means.idxmin()
            max_pos = list(means.index).index(max_idx)
            min_pos = list(means.index).index(min_idx)
            ...
            ax_bar.set_ylim(0, 255)
            ax_bar.set_yticks(np.arange(0, 256, 10))
            ax_bar.set_ylabel("Media (nivel de gris)")
            ax_bar.set_xticks(range(len(means)))
            ax_bar.set_xticklabels(means.index, rotation=45, ha="right")
            ...
            # -------------------------
            # 5) Histogramas individuales (selector) con escala uniforme
            # -------------------------
            st.markdown("### Histograma por columna")
            col_hist = st.selectbox("Selecciona columna", options=sel_cols, key=f"hist_{fname}")
            if col_hist:
                fig_h, ax_h = plt.subplots(figsize=(10, 4))
                vals = df0[col_hist].dropna().to_numpy(dtype=float)
                n, bins, patches = ax_h.hist(vals, bins=np.arange(-0.5, 256.5, 1), color="#90CAF9", alpha=0.9, edgecolor="k")
                
                # Marcar máximo y mínimo
                max_bin_idx = np.argmax(n)
                min_bin_idx = np.argmin(n[n > 0]) if (n > 0).any() else 0
                patches[max_bin_idx].set_edgecolor('green')
                patches[max_bin_idx].set_linewidth(2)
                if min_bin_idx < len(patches):
                    patches[min_bin_idx].set_edgecolor('red')
                    patches[min_bin_idx].set_linewidth(2)
                
                ax_h.set_xlim(0, 255)
                ax_h.set_xticks(np.arange(0, 256, 50))
                ax_h.set_xlabel("Nivel de gris (0–255)")
...
if len(data) >= 2:
    ...
    if common_cols_list:
        ...
        ax_kde_comp.set_yticks(np.arange(0, y_max + 10, 10))
